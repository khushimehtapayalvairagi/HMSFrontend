...
const ProcedureForm = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { patientId: initialPatientId, ipdAdmissionId: initialAdmissionId } =
    location.state || {};
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // fetch all admissions for each patient and keep only active ones
      const admittedPatients = [];
      const admissionsMap = {};

      await Promise.all(
        allPatients.map(async (p) => {
          const res = await axios.get(`${BASE_URL}/api/ipd/admissions/${p._id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          const foundAdmissions = res.data.admissions || [];

          const activeAdmissions = foundAdmissions.filter(
            (adm) => adm.status === 'Admitted'
          );

          if (activeAdmissions.length > 0) {
            admittedPatients.push(p);
            admissionsMap[p._id] = activeAdmissions;
          }
        })
      );

      setPatients(admittedPatients);
      setIpdAdmissions(admissionsMap);

      // get other data
      const [procRes, docRes, roomRes, lrRes] = await Promise.all([
        axios.get(`${BASE_URL}/api/receptionist/procedures`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        ...
        axios.get(`${BASE_URL}/api/receptionist/labour-rooms`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handlePatientChange = (e) => {
    ...
    // set related admissions for selected patient
    setIpdAdmissions((prev) => prev); // admission map already loaded
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${BASE_URL}/api/procedures/schedules`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div style={{ maxWidth: 600, margin: 'auto', padding: '2rem' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {formData.patientId && (
          <select
            name="ipdAdmissionId"
            value={formData.ipdAdmissionId}
            onChange={handleChange}
          >
            <option value="">Select Admission</option>
            {ipdAdmissions[formData.patientId]?.map((a) => (
              <option key={a._id} value={a._id}>
        ...
        {formData.procedureType === 'Labour Room' && (
          <select name="labourRoomId" onChange={handleChange}>
            <option value="">Select Labour Room</option>
            {labourRooms.map((r) => (
              <option key={r._id} value={r._id}>
                {r.name}
